    ...
    for (const auto &[class_name, class_index] : class_to_index) {
        for (const auto &p : fs::directory_iterator(directory + "/" + class_name)) {
            if (samples.size() >= max_samples) {
                return samples;  // Stop adding samples once the limit is reached
            }
...
ImageFolderDataset::ImageFolderDataset(const std::string &root, Mode mode, torch::IntArrayRef image_load_size,  size_t max_samples)
    : mode_(mode),
      ...
      samples_(create_samples(mode_dir_, class_to_index_, max_samples)) {}
